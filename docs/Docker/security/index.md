...
# Best Practices по безопасности при работе с Docker и Docker Compose

| Практика | Категория | Описание |
|----------|-----------|----------|
| Использовать минимальные образы | Images | Выбирай образы с суффиксом `-slim` или `-alpine` для минимизации поверхности атаки |
| Сканировать образы на уязвимости | Security Tools | Используй `trivy`, `grype`, `snyk` для анализа уязвимостей |
| Избегать запуска контейнеров от root | Users | Настраивай `USER` внутри Dockerfile, создавая отдельного пользователя |
| Минимизировать доступ к сокету Docker | Permissions | Оставляй доступ только группе `docker`, не выдавай `chmod 777` на `/var/run/docker.sock` |
| Использовать `docker-compose.override.yml` | Config Separation | Вынеси небезопасные настройки (например, проброс портов, dev-сервисы) в отдельный override-файл |
| Ограничивать ресурсы контейнеров | Runtime | Используй параметры `--cpus`, `--memory`, чтобы ограничить потребление ресурсов |
| Использовать secrets вместо env | Secrets | Храни пароли и ключи в Docker secrets, а не в переменных окружения |
| Регулярно обновлять образы | Maintenance | Пересобирай контейнеры с флагом `--pull`, чтобы получать свежие пакеты |
| Проверять безопасность хоста | Hardening | Используй `Docker Bench for Security` для анализа конфигурации системы |
| Ограничивать сеть | Networking | Создавай отдельные сети для сервисов, не подключай лишние контейнеры |
| Подписывать образы | Integrity | Используй `docker trust` и `Notary` для проверки цифровых подписей |
| Логировать и мониторить | Monitoring | Используй `Falco`, `sysdig`, `Prometheus` для обнаружения аномалий |
